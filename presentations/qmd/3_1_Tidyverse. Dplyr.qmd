...
msleep <- read_csv("./data/msleep.csv", show_col_types = FALSE)
```

## Data. Mammals sleep

Data: The **msleep** (mammals sleep) data set contains the sleeptimes and weights for a set of mammals and is available in the [dagdata](https://github.com/genomicsclass/dagdata) repository on github. This data set contains `r nrow(msleep)` rows and `r ncol(msleep)` variables.
...
## The columns of the dataset: {.smaller}
...
## Important dplyr verbs to remember {.smaller}
...
msleep |> 
    mutate(rem_proportion = sleep_rem / sleep_total, 
           ...
           wt_ge_50 = if_else(bodywt >= 50, TRUE, FALSE)) |>
    select(name, sleep_rem, sleep_total, 
           rem_proportion, bodywt, bodywt_grams, wt_ge_50) |>
    head()
```
